...
def degree(request):
    ...
    merged_all = pd.merge(merged_data, student_df, how='right', on='unitid')
...
def tuition(request):
    ...
    pivoted_df.columns = ['unitid', 'instnm', 'item', 'income_pc', "2021", "2022", "2023" ]

    # Calculate the growth between years with condition to avoid divide by zero
    pivoted_df['gr22'] = pivoted_df.apply(
        lambda row: ((row['2022'] - row['2021']) / row['2021'] * 100) if row['2021'] != 0 else None,
        axis=1
    )
    pivoted_df['gr23'] = pivoted_df.apply(
        lambda row: ((row['2023'] - row['2022']) / row['2022'] * 100) if row['2022'] != 0 else None,
        axis=1
    )

    pivoted_df['to_ic'] = pivoted_df.apply(
        lambda row: ((row['2023'] ) / row['income_pc'] * 100) if row['income_pc'] != 0 else None,
        axis=1
    )
    ...
    pivoted_df['rankings_tuition'] = pivoted_df.groupby('item')['2023'].rank(ascending=False)
    top10_rankings = pivoted_df.groupby('item').apply(lambda x: x.nlargest(10, '2023')).reset_index(drop=True)
    lowest10_rankings = pivoted_df.groupby('item').apply(lambda x: x.nsmallest(10, '2023')).reset_index(drop=True)
