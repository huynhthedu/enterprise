...
def latest(request):
    ...
    # queryset = VnData.objects.values()
    ...
    # # Filter out rows where value is null
    # df = df[df['value'].notnull()]

    # Get the current year and the previous year dynamically
    year2 = datetime.now().year
    year1 = year2 - 1

    # 2. Fetch all data from the database
    data = list(VnData.objects.values('id', 'group', 'year', 'value'))

    # 3. Convert to DataFrame
    df = pd.DataFrame(data)

    # 4. Get the latest and second latest values for each id and group combination
    latest_values = df.sort_values(['id', 'group', 'year']).groupby(['id', 'group']).last().reset_index()
    second_latest_values = df.sort_values(['id', 'group', 'year']).groupby(['id', 'group']).nth(-2).reset_index()

    # 5. Assign years dynamically
    latest_values['year'] = year2  # Assign latest values to 2025
    second_latest_values['year'] = year1  # Assign second latest values to 2024

    # 6. Combine the latest and second latest values for each id and group
    df_final = pd.concat([latest_values, second_latest_values], ignore_index=True)

    # 7. Drop unnecessary index columns and ensure correct structure (no 'id_key')
    df_final = df_final[['id', 'group', 'year', 'value']]
    ...
    # Print the DataFrame with the condition
    # print(df[df['year'] == year2])
    ...
    provinces = VnProvince.objects.values_list('province', flat=True).distinct().order_by('province')
    years = list(range(2009, 2026))  # Valid years range
    indicators = list(VnIndicators.objects.filter(weight__gt=0).values_list('indicators', flat=True).distinct().order_by('group'))
    ...
    return render(request, 'vietnam/latest.html', context)